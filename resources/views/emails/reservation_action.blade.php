...
<body
    ...
    <div
        ...
        {{-- Header --}}
        <div
            style="background-color: {{ $actionStatus === 'confirmed' ? config('customization.colors.secondary') : '#DC2626' }}; background-image: linear-gradient(135deg, {{ $actionStatus === 'confirmed' ? config('customization.colors.secondary') : '#DC2626' }} 0%, {{ $actionStatus === 'confirmed' ? config('customization.colors.secondary') : '#991B1B' }} 100%); color: #ffffff; padding: 50px 40px; text-align: center;">
            <div style="font-size: 50px; margin-bottom: 15px;">{{ $actionStatus === 'confirmed' ? '🎉' : '😓' }}</div>
            <h1 style="margin: 0; font-size: 26px; font-weight: 800; letter-spacing: -0.5px; color: {{ config('customization.colors.beige') }};">
        ...
        <div style="padding: 40px; color: #1B1B1E; text-align: center;">
            ...
            @if ($actionStatus === 'confirmed')
                <div
                    ...
                    <div style="margin-bottom: 15px; border-bottom: 1px solid #E5E7EB; padding-bottom: 10px;">
                        ...
                        <span
                            style="display: inline-block; background-color: {{ config('customization.colors.primary') }}; color: #ffffff; padding: 2px 10px; border-radius: 9999px; font-size: 14px; font-weight: 700;">{{ $reservation->persons }}</span>
                    </div>
                    <div>
                        <span
                        ...
                        <span
                            style="font-size: 14px; color: #111827; font-weight: 600;">{{ config('customization.address') }}</span>
                    </div>
                </div>
            ...
            @else
                ...
                <div style="margin-top: 40px;">
                    <a href="tel:+34{{ config('customization.phone') }}"
                        style="display: inline-block; background-color: {{ config('customization.colors.primary') }}; color: #ffffff; padding: 16px 32px; border-radius: 12px; text-decoration: none; font-weight: 700; font-size: 16px; box-shadow: 0 4px 14px rgba(114, 32, 34, 0.3);">
